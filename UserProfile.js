...
function UserProfile({ user }) {
    ...
    const loadUserProfile = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/users/${user._id || user.id}/profile`);
            if (response.ok) {
    ...
    const loadUserStats = async () => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/trips/customer/${user._id || user.id}`);
            if (response.ok) {
                const data = await response.json();
                if (data.success) {
